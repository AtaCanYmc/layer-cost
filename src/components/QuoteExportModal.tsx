  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#070b14]/85 backdrop-blur-md animate-fadeIn">
      <div className="bg-white text-slate-900 rounded-3xl w-full max-w-2xl shadow-2xl overflow-hidden flex flex-col max-h-[92vh]">
        ...
        <div className="no-print flex items-center justify-between px-6 py-4 bg-[#0c1220] text-white border-b border-slate-800">
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 rounded-xl bg-emerald-500/20 text-emerald-400 flex items-center justify-center">
              <FileCheck className="w-4 h-4" />
            </div>
            <h2 className="text-sm font-extrabold">3D Baskı Fiyat Teklifi / Proje Özeti</h2>
          </div>
        ...
        <div className="p-8 sm:p-10 overflow-y-auto print-card space-y-6 text-slate-800">
          ...
          <div className="flex justify-between items-start border-b border-slate-200 pb-6">
            <div>
              ...
              <p className="text-xs text-slate-500 font-medium">Hızlı & Hassas Üretim Çözümleri</p>
            </div>

            <div className="text-right text-xs text-slate-600 space-y-1 font-medium">
              <div><strong className="text-slate-900">Tarih:</strong> {today}</div>
              <div><strong className="text-slate-900">Teklif No:</strong> {quoteNumber}</div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4 bg-slate-50 p-4 rounded-2xl border border-slate-200 text-xs">
            <div>
              ...
              <div className="font-extrabold text-sm text-slate-900">{inputs.projectName || '3D Model Baskı İşi'}</div>
          ...
          {showInternalCosts && (
            <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 text-xs space-y-1.5 font-medium">
              <div className="font-extrabold text-amber-900 mb-1">Maliyet Dökümü (Yalnızca Şirket İçi)</div>
          ...
          <div className="bg-slate-950 text-white rounded-2xl p-5 flex items-center justify-between">
            <div>
              ...
              <span className="text-[11px] text-slate-400">Vergiler ve kargo hariçtir</span>
            </div>
          ...
          <div className="text-[10px] text-slate-500 text-center border-t border-slate-200 pt-4">
            Bu teklif belgesi 15 gün süreyle geçerlidir. 3D baskı parçaları siparişe özel üretilmektedir.
          </div>
        </div>
      </div>
    </div>
